...
import { loadState, saveState, upsertDraft, type Draft } from "../state/state";
...
function weekRange(today: string): { key: string; start: string; end: string } {
  ...
  return { key: isoWeekKey(ymd), start: formatYmd(start), end: formatYmd(addDays(start, 6)) };
}
...
async function loadEpisode(d: Draft): Promise<LongEpisode> {
  ...
  return { draftId: d.id, chapterTitle: p.script.title, script: p.script, timeline: p.timeline, audio: { ...p.audio, voice: `drafts/${d.id}/voice.wav` } };
}
...
async function main() {
  ...
  if (opts.dryRun) {
    ...
    if (state.weeklyCompiled.includes(week.key)) {
      log.info(`Week ${week.key} already compiled. Nothing to do.`);
      return;
    }
    const inWeek = state.drafts.filter((d) => d.kind === "short" && (d.status === "approved" || d.status === "uploaded") && (!week.start || (d.date >= week.start && d.date <= week.end)));
    if (inWeek.length < cfg.weeklyMinApproved) {
      log.info(`Only ${inWeek.length} approved Shorts in ${week.key} (need ${cfg.weeklyMinApproved}). Skipping.`);
      return;
    }
  ...
  const props: LongVideoProps = { channel, weekTitle: lang.weeklyTitle, outro: lang.weeklyOutro, episodes, music, musicVolume: cfg.music.volume * 0.85, duckedVolume: cfg.music.duckedVolume };
  ...
  const title = `${lang.weeklyTitle}: ${episodes.map((e) => e.chapterTitle.replace(/\?$/, "")).slice(0, 2).join(" · ")}${episodes.length > 2 ? " and more" : ""}`.slice(0, 100);
  const description = `${cfg.characterName} answers ${episodes.length} big questions this week.\n\n${chapters}\n\n${episodes.map((e) => `• ${e.chapterTitle}`).join("\n")}`;
  await writeJsonAtomic(path.join(dir, "meta.json"), { title, description, chapters, durationSeconds, includes });
  log.info(`Compilation: ${episodes.length} episodes, ${durationSeconds.toFixed(0)}s\n${chapters}`);

  const thumbProps: ThumbnailProps = { channel, words: thumbnailWords(episodes[0]!.chapterTitle), background: episodes[0]!.script.background };
  const thumbnail = await renderPng({ compositionId: "Thumbnail", inputProps: thumbProps, outputPath: path.join(dir, "thumbnail.png") });
  const video = path.join(dir, "long.mp4");
  if (!opts.skipRender) await renderVideo({ compositionId: "LongVideo", inputProps: props, outputPath: video, crf: 21 });
  ...
  let draft: Draft = {
    ...
    includes,
    description,
    tags: cfg.youtube.longTags,
  };
  if (releasesConfigured()) {
    const stored = await storeDraft({ tag: `weekly-${week.key}`, title, notes: description, video, script: path.join(dir, "meta.json"), thumbnail, props: path.join(dir, "long.json") });
    draft = { ...draft, releaseTag: stored.release.tag_name, releaseUrl: stored.release.html_url, assets: { video: stored.video.url, videoPublic: stored.video.browser_download_url, script: stored.script.url, thumbnail: stored.thumbnail?.url, props: stored.props?.url } };
  }
  let preview = video;
  if ((await fs.stat(video)).size > TELEGRAM_UPLOAD_LIMIT_BYTES - 2 * 1024 * 1024) {
    ...
    await renderVideo({ compositionId: "LongVideo", inputProps: props, outputPath: preview, scale: 0.5, crf: 32 });
  }
